 ...
 *
 */
public class ReadAttributesResponseImpl extends ResponseImpl implements
        ReadAttributesResponse {

    private ReadAttributesStatus[] attributes;

    public ReadAttributesResponseImpl(Response response,
            AttributeDescriptor[] descriptors) throws ZigBeeClusterException {
        super(response);
        ResponseImpl.checkGeneralCommandFrame(response,
                ReadAttributesResponse.ID);
        attributes = new ReadAttributesStatus[descriptors.length];
        byte[] msg = getPayload();
        ZBDeserializer deserializer = new DefaultDeserializer(msg, 0);
        for (int i = 0; i < descriptors.length; i++) {
            attributes[i] = new ReadAttributeStatusImpl(descriptors[i],
                    deserializer);
        }
    }
