...
public class AttributeActionPanel extends JPanel {
	...
	class PanelLogReportListener implements ReportListener {
		...
		public void receivedReport(Dictionary<Attribute, Object> reports) {
			...
			while (attributes.hasMoreElements()) {
				...
				LogPanel.log("Received Event from device " + device.getName()
	...
	public AttributeActionPanel() {
		...
		buildButtonPanel();
		add(new JScrollPane(getInputText()),
				Util.setConstrains(0, 1, 1, 1, 100, 100));
	...
	private JButton getWriteButton() {
		...
		doAction.addActionListener(new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					try {
						newValue = Converter.fromString(getInputText()
								.getText(), attribute.getZigBeeType());
					} catch (NumberFormatException ex) {
						LogPanel.log("Invalid value for " + attribute.getName()
								+ " we where expecting a number."
								+ "\n\tStatus: SKIPPED");
						return;
					}
					attribute.setValue(newValue);
					LogPanel.log("Set Attribute " + attribute.getName()
							+ " to " + newValue + "\n\tStatus: SUCCESS\n");
		...
		doAction.addActionListener(new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					LogPanel.log("Read Attribute " + attribute.getName()
							+ "\n\tStatus: SUCCESS\n\tResult: " + result + "\n");
				} catch (ZigBeeClusterException ex) {
					LogPanel.log("Read Attribute " + attribute.getName()
							+ "\n\tStatus: FAILED\n\tException: " + ex + "\n");
	...
	private JButton getSubscribeButton() {
		...
		doAction.addActionListener(new AbstractAction() {
			...
			public void doSubscribe(ReportListener listener) {
				...
				try {
					sub.setMaximumReportingInterval(Integer.parseInt(maxText
							.getText()));
				} catch (NumberFormatException ex) {
					LogPanel.log("The value '"
							+ maxText.getText()
							+ "' typed in the Maximum field is not a number"
							+ "Using default value "
							+ Subscription.DEFAULT_MAX_REPORTING_INTERVAL
							+ ", "
							+ "that means report the value at least once every "
							+ Subscription.DEFAULT_MAX_REPORTING_INTERVAL
							+ " seconds");
					sub.setMaximumReportingInterval(Subscription.DEFAULT_MAX_REPORTING_INTERVAL);
				}
				try {
					sub.setMinimumReportingInterval(Integer.parseInt(minText
							.getText()));
				} catch (NumberFormatException ex) {
					LogPanel.log("The value '"
							+ minText.getText()
							+ "' typed in the Minimum field is not a number."
							+ "Using default value "
							+ Subscription.DEFAULT_MIN_REPORTING_INTERVAL
							+ ", "
				...
				if (sub instanceof AnalogSubscription) {
					...
					try {
						...
						LogPanel.log("The value '"
								+ changeText.getText()
								+ "' typed in the Delta field is not a number."
								+ "Using default value "
				...
				if (attribute.getSubscription().addReportListener(listener)) {
					LogPanel.log("Subscribed to " + attribute.getName()
							+ "\n\t" + "Status: SUCCESS\n");
			...
			public void doUnsubscribe(ReportListener listener) {
				if (attribute.getSubscription().removeReportListner(listener)) {
					LogPanel.log("Unsubscribed to " + attribute.getName()
							+ "\n\t" + "Status: SUCCESS\n");
			...
			private void updateCategory(String category) {
				HADeviceTreeNode node = (HADeviceTreeNode) Mediator
						.getUPnPDeviceTree().getLastSelectedPathComponent();
	...
	private JPanel getSubscriptionPanel() {
		...
		subscriptionPanel.add(new JLabel("Reporting Configuration",
				JLabel.CENTER), Util.setConstrains(0, 0, 3, 1, 1, 1));

		subscriptionPanel.add(new JLabel("Minimum", JLabel.CENTER),
				Util.setConstrains(0, 1, 1, 1, 1, 1));
		subscriptionPanel.add(new JLabel("Maximum", JLabel.CENTER),
				Util.setConstrains(1, 1, 1, 1, 1, 1));
		subscriptionPanel.add(new JLabel("Delta", JLabel.CENTER),
				Util.setConstrains(2, 1, 1, 1, 1, 1));
	...
	private void buildButtonPanel() {
		...
		buttonPanel.add(getSubscribeButton(),
				Util.setConstrains(2, 0, 1, 1, 1, 1));
		buttonPanel.add(getSubscriptionPanel(),
				Util.setConstrains(0, 1, 3, 1, 1, 1));
	}
