 ...
 *
 */
public class EHDriverConfiguration implements ManagedService, ReportingConfiguration {

    public final static String PID = "it.cnr.isti.zigbee.eh.configuration";

    private final static Logger logger = LoggerFactory.getLogger(EHDriverConfiguration.class);
     ...
     * creation behavior of the {@link EHDeviceBase} objects, by following the rules:<br>
     * <ul>
     * <li>
     ...
     * cluster available X available for input too, and it add it to the actual list of cluster for the {@link EHDevice} created
     * <li>
     * if the value is equal to <b>ProfileStrict</b> then the <i>Home Automation Driver</i> create the cluster for the<br>
     * {@link EHDevice} if and only if it is reported as an <i>Input cluster</i>
     * </li>
     *
     */
    public static final String PROVIDED_CLUSTER_MODE_KEY = "it.isti.cnr.zigbee.eh.driver.cluster.discovery.mode";
    /**
     * The default value for the property {@link #PROVIDED_CLUSTER_MODE_KEY}, that is <b>ProfileStrict</b>
     */
    public static final String DEFAULT_PROVIDED_CLUSTER_MODE = ProvidedClusterMode.ProfileStrict.toString();

    private final HashMap<String, Object> configuration = new HashMap<String, Object>();
    private final BundleContext context;

    public EHDriverConfiguration(BundleContext bc){
        context = bc;
        synchronized (configuration) {
            ...
            try{
                configuration.put(PROVIDED_CLUSTER_MODE_KEY,
                    OSGiProperties.getString(context, PROVIDED_CLUSTER_MODE_KEY, ProvidedClusterMode.ProfileStrict.toString() )
                );
                ...
                logger.error(
                    ...
                    new Object[]{
                        ...
                        ProvidedClusterMode.ProfileStrict.toString()
                    }
                );
                configuration.put(PROVIDED_CLUSTER_MODE_KEY, ProvidedClusterMode.ProfileStrict.toString() );
            }
        }
