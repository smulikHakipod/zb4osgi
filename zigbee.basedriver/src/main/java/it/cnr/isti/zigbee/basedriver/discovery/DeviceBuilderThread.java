...
public class DeviceBuilderThread implements Stoppable {
	...
	private void inspectNode(ZToolAddress16 nwkAddress,
		...
		if (isNew) {
			...
			if (!changedNetworkAddress(node, nwk)) {
				 ...
				 */
				inspectDeviceOfNode(nwk,
						new ZigBeeNodeImpl(nwk, node.getIEEEAddress()));
			}
	...
	private boolean changedNetworkAddress(ZigBeeNodeImpl node, int nwk) {
		...
		/*
		 * //TODO For covering case B: we should compare the content of
		 * "registrations" with the actual list of End Point on the node
		 */
		for (TByteObjectHashMap<ServiceRegistration> registration : ((Map<String, TByteObjectHashMap<ServiceRegistration>>) registrations)
				.values()) {
			final ZigBeeDeviceImpl device = (ZigBeeDeviceImpl) Activator
					.getBundleContext()
					.getService(
							((ServiceRegistration) registration).getReference());
			if (device.setPhysicalNode(new ZigBeeNodeImpl(nwk, node
					.getIEEEAddress()))) {
				changed = true;
				((ServiceRegistration) registration).setProperties(device
						.getDescription());
			}
		}
