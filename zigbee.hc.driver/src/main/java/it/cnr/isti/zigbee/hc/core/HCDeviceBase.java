...
public abstract class HCDeviceBase implements HCDevice  {
    ...
    protected GenericTunnel genericTunnel;
    protected ProtocolTunnel11073 tunnel11073;
    /*
    ...
    protected Time time;
    protected RSSILocation rssiLocation;
    protected Commissioning commissioning;
    ...
    public HCDeviceBase(BundleContext ctx, ZigBeeDevice zbDevice ) throws ZigBeeHCException{
        ...
        identify = (Identify) addCluster(HCProfile.IDENTIFY);
        genericTunnel = (GenericTunnel) addCluster(HCProfile.GENERIC_TUNNEL);
        tunnel11073 = (ProtocolTunnel11073) addCluster(HCProfile.PROTOCOL_TUNNEL_11073);

        powerConfiguration = (PowerConfiguration) addCluster(HCProfile.POWER_CONFIGURATION);
        alarms = (Alarms) addCluster(HCProfile.ALARMS);
        time = (Time) addCluster(HCProfile.TIME);
        rssiLocation = (RSSILocation) addCluster(HCProfile.RSSI_LOCATION);
        commissioning = (Commissioning) addCluster(HCProfile.COMMISSIONING);
    }
