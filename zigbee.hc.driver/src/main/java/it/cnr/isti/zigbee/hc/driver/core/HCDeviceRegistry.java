...
public class HCDeviceRegistry {
        ...
        try {
            ...
            if (zbDeviceSRs!= null){
                for (int i= 0; i<zbDeviceSRs.length;i++){
                    addHCDeviceFactory(zbDeviceSRs[i]);
                }
            }
    ...
    public void addHCDeviceFactory(ServiceReference zbDeviceFactorySR) {
    ...
    public void  removeHCDeviceFactory(ServiceReference zbDeviceFactorySR) {
	...
	public HCDeviceFactory getExactFactory(ZigBeeDevice zbDevice) {
		
		final String deviceId = String.valueOf(zbDevice.getDeviceId());
		
	        synchronized (REGISTRY) {
	        	HCDeviceFactory exact = registry.get(deviceId);
	            if  (exact != null) 
	        		return exact;
        		return registry.get(UnknowHCDevice.DEVICE_ID);
	        }
