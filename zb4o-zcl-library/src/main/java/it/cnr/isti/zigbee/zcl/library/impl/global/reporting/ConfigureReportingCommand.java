...
public class ConfigureReportingCommand extends AbstractCommand {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(ConfigureReportingCommand.class);
	...
	public ConfigureReportingCommand(
			AttributeReportingConfigurationRecord[] attributerecord) {
		super(ID, false);
		...
		if (payload == null) {
			...
			for (int i = 0; i < attributerecord.length; i++) {
				if (attributerecord[i].getDiretion() == 1) {
					...
					// CASE OF ATTRIBUTE CONFIGURATION SENT TO SERVER OF A
					// DISCRETE ATTRIBUTE
					// Size of: Direction + Attribute Id + Data Type + Minimum +
					// Maxium
					length += 1 + 2 + 1 + 2 + 2;
				} else {
					// CASE OF ATTRIBUTE CONFIGURATION SENT TO SERVER OF A
					...
					// Size of: Direction + Attribute Id + Data Type + Minimum +
					// Maxium + Change
					final int valueSize = attributerecord[i]
							.getAttributeDataType().getLength();
			...
			for (int i = 0; i < attributerecord.length; i++) {
				if (attributerecord[i].getDiretion() == 1) {
					// CASE OF ATTRIBUTE CONFIGURATION SENT TO CLIENT
					// Size of: Direction + Attribute Id + Timeout
					serializer.append_byte((byte) attributerecord[i]
							.getDiretion());
					serializer.append_short((short) attributerecord[i]
							.getAttributeId());
					serializer.append_short((short) attributerecord[i]
							.getTimeoutPeriod());
				} else if (attributerecord[i].getAttributeDataType().isAnalog() == false) {
					...
					serializer.append_short((short) attributerecord[i]
							.getAttributeId());
					serializer.append_byte((byte) attributerecord[i]
							.getAttributeDataType().getId());
					serializer.append_short((short) attributerecord[i]
							.getMinimumReportingInterval());
					serializer.append_short((short) attributerecord[i]
							.getMaximumReportinInterval());
				} else {
					// CASE OF ATTRIBUTE CONFIGURATION SENT TO SERVER OF A
					// ANALOG ATTRIBUTE
					// Size of: Direction + Attribute Id + Data Type + Minimum +
					...
					serializer.append_short((short) attributerecord[i]
							.getAttributeId());
					final ZigBeeType type = attributerecord[i]
					...
					serializer.append_short((short) attributerecord[i]
							.getMaximumReportinInterval());
					serializer.appendZigBeeType(
							attributerecord[i].getReportableChange(), type);
				}
			}
